...
このCHANGELOGは、公開tag/releaseと、単独tagを持たない内部開発snapshotの両方を含みます。公開ユーザー向けの最新リリースでは、日本語の要約を先に置き、その後にEnglish summaryを続けます。PR #439のRelease Candidate版は v2.2.1045 であり、最終成果物のSHA256はactual main commitから再生成・検証した後、GitHub Releaseへ記録します。

This changelog includes both published tags/releases and internal development snapshots that did not receive standalone tags. For current user-facing releases, each entry starts with Japanese notes followed by an English summary. The PR #439 release-candidate version is v2.2.1045; final artifact SHA256 values are generated and verified from the actual main commit before being recorded in the GitHub Release.

## v2.2.1045 (2026-09-01) — Release Candidate: K2/CFS monitoring hardening and namespace-aware Creality errors
...
- **K2/CFS監視UI**: 外部スプールと CFS 0-4台を分け、slot装填、選択、色、残量、fresh/staleをread-onlyで確認できる表示を維持します。
- **CFS guarded print-start**: K2/CFSの印刷開始前に、装填済みsourceの選択状態が異常または未観測でないかを検査し、安全に確定できない場合は送信前に停止します。
- **CFS復旧ラッチと並行送信ガード**: 物理CFS操作候補は、未解決復旧ラッチ、破損/衝突record、同一deviceの古い未解決command、同一deviceの並行dispatchをfail-closedで止めます。
...
- **K2/CFS monitoring UI**: keeps the external spool and 0-4 CFS units separated, with read-only slot presence, selection, color, remaining amount, and fresh/stale state.
- **CFS guarded print start**: K2/CFS print-start preflight stops before sending when loaded source selection evidence is invalid or unobserved.
- **CFS recovery latch and concurrent-send guard**: physical CFS command candidates fail closed on unresolved recovery records, corrupted/conflicting records, older unresolved commands on the same device, and concurrent dispatch on the same device.
...
> **注意**: フィラメント帰属の入力を変更する**実機検証候補**。オフライン完了印刷の遡及紐付けが正しい装着区間で
> 絞られるか（過剰/過少紐付けが無いか）を実機で確認してから正式リリースすること。
> ※ 当初 v2.2.1036 として作成した P1-3 を、CPU修正 v2.2.1037 が先に main へ入ったため、最新 main(1037) 上へ
>   cherry-pick し直し v2.2.1038 として再構成したもの（内容は同一）。

- **P1-3**: `autoCorrectCurrentSpool` の「オフライン中に完了した印刷を現在スプールへ遡及紐付け」する下限を、
  legacy `spool.startPrintID` から **mountHistory の open 区間 `sinceJobId`（ADR-0004 権威台帳、`getSpoolIntervals`）**
  へ変更。`startPrintID` が編集等でドリフトした場合の誤紐付け（古い境界で過剰にリンク）を防ぐ。mount 記録が
  無い旧データのみ `startPrintID` へフォールバック。
- テスト: 新規2（mountHistory 境界で絞る／記録なし時フォールバック）。全**795**テスト緑・eslint 0 error。

---

## v2.2.1037 (2026-07-11) — CPU負荷・カメラライフサイクル修正（公開済み・Latest）
...
#### 新機能 / 修正（不具合）
- **「最新ではないのに印刷中（未確定）のまま放置」された履歴ジョブを中止確定として明示。** より新しいジョブID（＝後続の印刷）が存在する以上、その未確定ジョブはもう継続されていない（中断/電源断/再起動等で完了報告が来なかった）と確定できる。これを内部データと表示の両方で明示する。
  - **非破壊**: `printfinish` は書き換えず `null`（未確定）のまま維持＝統計の成否集計対象外を堅持。別フラグ **`discontinued: true`** のみを立てる。`finishTime` も捏造しない。
  - **表示**: 無期限の「…（未確定）」ではなく **「⏹（中止）」**（`result-aborted`、失敗の赤と区別した山吹色）＋理由ツールチップ。
  - **保護対象**: 最新（id 最大）のジョブと、稼働中（`currentPrintID` 一致）のジョブは中止にしない（再開報告待ちの可能性）。
  - **自己修復**: 後から完了報告（`finishTime`）が届けば `discontinued` を自動解除。
- **外部連携（ItemKeeper）へも伝播**: 送信ジョブに `discontinued: true` を additive 追加（true のときのみ）。`state`/`result`/`printfinish` は不変＝完全な後方互換。受信側は `state:"printing"` かつ `discontinued:true` を「中止（継続なし）」として扱える。スキーマ番号（`3dpmon.ik.history.v1`）は据え置き。
- リレー親子（サテライト）はジョブオブジェクト丸ごとミラーのため `discontinued` も同梱して伝播。

#### テスト
- 全739テスト緑（新規16: 判定純関数/merge統合の中止検知・保護・自己修復/実DOMでの ⏹ レンダリング/jobsToRaw 引き継ぎ/ItemKeeper additive 送信）。
