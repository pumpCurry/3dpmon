...
- Gate 18.9Gでは、module-owned trusted result-set completeness registryを追加した。registryがdevice/job/plan/source集合の完全一致を証明した場合だけ、shadow print binding repositoryは未出現sourceを`confirmed-unused`へ分類できる。これはsource-aware usage attributionの信頼境界であり、production spool debit、legacy `usageHistory`、spool残量更新はまだ行わない。
