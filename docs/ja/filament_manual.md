...
本書では、3dpmon に搭載されている「フィラメント使用記録簿」「在庫管理」「プリセット管理」の3機能について説明します。印刷時のスプール交換や在庫把握に役立つダッシュボード機能です。
...
| フィールド | スコープ | 説明 |
| --- | --- | --- |
| `filamentSpools` | グローバル | 全スプールの定義・残量情報。全プリンタで共有 |
| `usageHistory` | グローバル | ジョブ単位の使用履歴。hostname フィールドでどのプリンタかを識別 |
...
```javascript
// 例: 2台のプリンタにそれぞれ異なるスプールを装着
monitorData.hostSpoolMap = {
  "K1Max-4A1B": "spool_1710000000000",  // 1台目: PLA白
  "K1Max-03FA": "spool_1710000001234"   // 2台目: PETG黒
};
```
...
- **usageHistory**: ジョブ単位の使用長さや開始終了時刻を記録します。`hostname` フィールドにより、どのプリンタでの印刷かを特定できます。
...
### per-host スプール交換の流れ
1. フィラメント管理モーダルを開きます
2. 対象プリンタを選択します (接続中のプリンタのみ表示)
3. 交換ボタンを押すとフィラメント選択画面が表示されます
4. お気に入りや最近使ったフィラメントから選択するか、ドロップダウンで絞り込みます
5. 決定すると在庫数が1減り、そのプリンタの装着スプールとして `hostSpoolMap` に登録されます

### per-host 使用量トラッキング
- 印刷中、各プリンタが報告する使用量は hostname に紐付けて記録されます
...
- **検索欄** ではブランド、材質、名前、色、名称を指定して絞り込めます。
  ...
  - 色: `{filamentColor}{materialColorName}`
  - 名称は部分一致で検索します。
- **検索ボタン** を押すと一覧が更新されます。
...
マルチプリンタ環境では、プリンタごとの消費量も確認可能です。

## 5. プリセット登録と使用フロー
1. **プリセットタブ**で新規登録します。名前・色・材質などを入力します。
2. 在庫数を追加すると、交換時にそのプリセットを選択できます。
3. スプール交換時には対象プリンタを指定し、在庫から選択します。残数が自動で更新されます。
4. 在庫が不足すると警告が表示されます。
...
- 残量から印刷可能時間を推定
- 廃棄記録やサムネイル画像の登録
- プリンタごとの使用量フィルタリング (接続中のプリンタのみ表示)
