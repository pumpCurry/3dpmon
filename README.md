...
- v2.1.012 で **フィラメントデータ整合性の根本再設計 + per-host接続トグル** を実施しました。アプリOFF中の印刷消費の遡及補正、スプール未装着時のゴースト表示修正、印刷中アイコン誤表示(✗→▶)修正、completionElapsedTime振動修正、リレー子クライアント画面更新修正、TTS起動時発声失敗修正、接続設定モーダル内per-host接続/切断トグル追加を含みます。変更履歴は [CHANGELOG.md](CHANGELOG.md) を参照してください。
## インストール
1. このリポジトリをダウンロードします。
