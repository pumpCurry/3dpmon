...
- v2.1.007 で **フィラメント管理拡張（Phase 5）** を実施しました。per-hostストレージ分離（6台運用対応）、色別印刷集計（ドーナツチャート付き）、残フィラメント活用提案（スコアリングエンジン + 印刷ダイアログ統合）を追加しました。151テスト全グリーン。変更履歴は [CHANGELOG.md](CHANGELOG.md) を参照してください。
## インストール
1. このリポジトリをダウンロードします。
