...
- **v2.1.017 (LTS)** — 単一機器時代のレガシーコードを完全駆除し、マルチホスト環境での安定性を確立したバージョンです。v2.1.009以前のデータ形式からの移行が必要な場合は、このバージョンをご利用ください。
- **v2.1.018 以降** では単一機器時代のデータ構造サポート（v1.x/v2.0 旧フォーマット）を終了します。旧バージョンからのアップグレードは必ず v2.1.017 経由で行ってください。
