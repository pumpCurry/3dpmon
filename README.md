...
- v2.1.004 で **マルチプリンタ間のデータ分離を全面監査・修正** しました。グローバルフォールバックによるプリンタ間データ漏洩を根絶し、スプール装着・カメラ ON/OFF・印刷ステート遷移・3Dプレビュー回転・ファイルリスト・温度グラフをすべて per-host 化しました。また **v1.25/v1.29 レガシー移行を廃止** し、最小サポート移行元を v1.40 に明記しました。変更履歴は [CHANGELOG.md](CHANGELOG.md) を参照してください。
## インストール
1. このリポジトリをダウンロードします。
