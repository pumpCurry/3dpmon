 ...
 *
 * @version {バージョン番号}        e.g. `1.390.0`
 * @since   {初出バージョン}        e.g. `v1.390.0`
 */
```
...
| `{バージョン番号}`  | リリースバージョン（SemVer 推奨）                    |
| `{初出バージョン}`  | 当該モジュールが最初に追加されたバージョン                   |

### 2.2 バージョンについて 
- 現在のメジャーバージョンとマイナーバージョンは 1.390です。
- リビジョン番号をコミット/プルリクエストの際プルリクエストのチケット番号にあわせてください。
