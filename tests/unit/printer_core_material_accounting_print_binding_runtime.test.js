...
describe("MaterialAccountingPrintBindingRuntime", () => {
  ...
  it("mount open後print-start前に同一sourceの変更イベントがある場合はdebit候補へ昇格しない", async () => {
    const data = createRuntimeData();
    attachOpenMounts(data);
    const plan = createPlan(data);
    const hostname = attachObservedPrintJob(data, { printJobId: "job:k2-source-changed-before-print-start" });
    const persist = vi.fn(async ({ nextStore }) => {
      data.materialAccountingPrintBindingStore = nextStore;
      return { ok: true, casApplied: true, backend: "test" };
    });
    ...
    deviceRecord.events = Array.isArray(deviceRecord.events) ? deviceRecord.events : [];
    deviceRecord.events.push({
      observationId: "mso:serial-k2:source-k2-cfs-1a:changed-before-print-start",
      deviceId: "serial:k2",
      sourceId: "source:k2:cfs:1a",
    ...
    await runtime.recordObservedPrintStart({ printPlan: plan, hostname, printJobId: "job:k2-source-changed-before-print-start" });
    attachObservedCompletedPrintJob(data, {
      hostname,
      printJobId: "job:k2-source-changed-before-print-start",
      completedAt: "2026-09-01T08:31:00.000Z",
    });
    markMaterialSourcesObservedAt(data, "2026-09-01T08:30:45.000Z");
    data.machines[hostname].printStore.history.at(-1).materialUsed = "3210,6543";
  ...
  it("nested spoolMount.openedAt改変ではmount open後のsource変更イベントを検査範囲外へ追い出せない", async () => {
    const data = createRuntimeData();
    attachOpenMounts(data);
    const plan = createPlan(data);
    const hostname = attachObservedPrintJob(data, { printJobId: "job:k2-tampered-nested-opened-at" });
    const persist = vi.fn(async ({ nextStore }) => {
      data.materialAccountingPrintBindingStore = nextStore;
      return { ok: true, casApplied: true, backend: "test" };
    });
    ...
    deviceRecord.events = Array.isArray(deviceRecord.events) ? deviceRecord.events : [];
    deviceRecord.events.push({
      observationId: "mso:serial-k2:source-k2-cfs-1a:changed-before-tampered-open",
      deviceId: "serial:k2",
      sourceId: "source:k2:cfs:1a",
      observedAt: "2026-09-01T07:45:00.000Z",
      changeKind: "source-changed",
    ...
    await runtime.recordObservedPrintStart({ printPlan: plan, hostname, printJobId: "job:k2-tampered-nested-opened-at" });
    const snapshots = data.materialAccountingPrintBindingStore.printStartSnapshots;
    const changedSnapshot = snapshots.find((snapshot) => snapshot.protocolToolAlias === "T1A");
    changedSnapshot.spoolMount = {
      ...(changedSnapshot.spoolMount || {}),
      openedAt: "2026-09-01T07:55:00.000Z",
    };
    attachObservedCompletedPrintJob(data, {
      hostname,
      printJobId: "job:k2-tampered-nested-opened-at",
      completedAt: "2026-09-01T08:31:00.000Z",
    });
    markMaterialSourcesObservedAt(data, "2026-09-01T08:30:45.000Z");
    data.machines[hostname].printStore.history.at(-1).materialUsed = "3210,6543";
  ...
  it("print-start受信と同一msのsource変更イベントは印刷中断絶としてdebit候補へ昇格しない", async () => {
    ...
    const plan = createPlan(data);
    const hostname = attachObservedPrintJob(data, {
      printJobId: "job:k2-source-changed-at-start",
    ...
    deviceRecord.events = Array.isArray(deviceRecord.events) ? deviceRecord.events : [];
    deviceRecord.events.push({
      observationId: "mso:serial-k2:source-k2-cfs-1a:changed-at-print-start",
      deviceId: "serial:k2",
      sourceId: "source:k2:cfs:1a",
      observedAt: "2026-09-01T08:01:05.000Z",
      changeKind: "source-changed",
      before: null,
      after: null,
      authority: "observation-only",
  ...
  it("source固有event coverageがmount open後からしか証明できない場合は該当sourceだけdebit候補へ昇格しない", async () => {
    ...
    attachOpenMounts(data);
    const plan = createPlan(data);
    const hostname = attachObservedPrintJob(data, { printJobId: "job:k2-source-event-coverage-gap" });
    const persist = vi.fn(async ({ nextStore }) => {
      data.materialAccountingPrintBindingStore = nextStore;
      return { ok: true, casApplied: true, backend: "test" };
    });
    ...
    attachObservedCompletedPrintJob(data, {
      hostname,
      printJobId: "job:k2-source-event-coverage-gap",
      completedAt: "2026-09-01T08:31:00.000Z",
      observedReceivedAt: "2026-09-01T08:31:05.000Z",
    });
    markMaterialSourcesObservedAt(data, "2026-09-01T08:30:45.000Z");
