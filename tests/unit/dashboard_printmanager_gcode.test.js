...
const { registerGcodeMetaForHosts, resolveHistoryFinishStatus, _mergeFilamentInfo,
  parseRawHistoryEntry, jobsToRaw } =
...
describe('printfinish 確定: 印刷中ジョブを成功/失敗へ誤確定しない（再起動時 誤計上防止）', () => {
  it('parseRawHistoryEntry: 終了時刻なし＝印刷中 → printfinish=null（usagetime>0でも成功にしない＝IR3v2 ✔ 誤確定防止）', () => {
    const raw = { id: 1781000001, filename: '/x/Piggy.gcode', starttime: 1781000001, usagetime: 600 /* >0 */ };
    const e = parseRawHistoryEntry(raw, 'http://127.0.0.1', 'IR3V2');
    expect(e.printfinish).toBeNull();
  });

  it('parseRawHistoryEntry: 終了時刻なし＋usagetime=0 → printfinish=null（K1 ✗ 誤確定防止）', () => {
    const raw = { id: 1781000002, filename: '/x/a.gcode', starttime: 1781000002, usagetime: 0 };
    const e = parseRawHistoryEntry(raw, 'http://127.0.0.1', 'K1Max');
    expect(e.printfinish).toBeNull();
  });
