/**
 * @fileoverview
 * dashboard_storage_ui.js (ver.1.327)
 *
 * ストレージ使用量・同期時刻・クォータ計測 UI を管理するコントローラ。
 * - ストレージ使用率の表示更新（定期 / イベント駆動）
 * - 保存ボタンによる明示的な同期処理
 * - クォータテストで localStorage の最大書込可能量を計測
 * - 全データ Export / Import もここで提供
 * - "storage:sync" / "storage:legacyCleaned" カスタムイベントへの応答
 * - すべてのフィードバックはパネル内に 3 秒だけ表示
 */
